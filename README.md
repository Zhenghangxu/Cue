...
Uninstalling or deleting the PWA must preserve the server's `~/.config/subtitle-maker/config.json`. Reinstalling uses the same saved settings and credentials. Configuration lives outside the browser profile, installed app, and `Application Support/Cue` directory. Start the updated backend once before removing the old app so any legacy config is copied. Cue never deletes either config file during installation, uninstallation, or cache cleanup; a separate cleanup utility or manual filesystem deletion remains outside its control.
